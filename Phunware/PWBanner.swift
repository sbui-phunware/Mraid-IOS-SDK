...
@objc public class PWBanner: NSObject {
    ...
    @objc func refresh() {
        if(self.placementRequestConfig == nil){
            NSLog("Phunware", "You must construct a PWBanner with a PlacementRequestConfig in order to support zone refreshing.")
            return;
        }
        ...
        if(self.placement != nil){
            ...
            Phunware.requestPlacement(with:self.placementRequestConfig!) { response in
                switch response {
